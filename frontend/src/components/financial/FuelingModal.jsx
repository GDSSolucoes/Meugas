...
export default function FuelingModal({
  ...
  const loadData = async () => {
    try {
      const [vehiclesData, employeesData, fuelingsData, groupsData] =
        await Promise.all([
  ...
  const searchVehicleByPlate = async (plate) => {
    ...
    const vehicle = vehicles.find(
      (v) => v.plate.toUpperCase().replace(/[^A-Z0-9]/g, "") === formattedPlate,
    );

    if (vehicle) {
      // Buscar último abastecimento do veículo
      const vehicleFuelings = fuelings.filter(
        (f) => f.vehicleId === vehicle.id,
      );
      const lastFueling =
        vehicleFuelings.length > 0 ? vehicleFuelings[0] : null;
  ...
  const validateForm = () => {
    ...
    if (!formData.currentKm || Number(formData.currentKm) <= 0) {
      toast({
        title: "Erro",
        description: "Km atual deve ser maior que zero.",
        variant: "destructive",
      });
      return false;
    }
    if (Number(formData.currentKm) <= formData.lastKm) {
      toast({
        title: "Erro",
        description: "Km atual deve ser maior que o último registro.",
        variant: "destructive",
      });
      return false;
    }
    if (!formData.liters || Number(formData.liters) <= 0) {
      toast({
        title: "Erro",
        description: "Quantidade de litros deve ser maior que zero.",
        variant: "destructive",
      });
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.createExpense && selectedAccount) {
        ...
        const combustivelGroup = groups.find(
          (g) =>
        ...
        toast({
          title: "Sucesso",
          description:
            "Abastecimento registrado e lançamento de despesa criado!",
        });
      ...
      toast({
        title: "Erro",
        description: "Falha ao salvar abastecimento.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleSelectFueling = (fueling) => {
    ...
    const previousFueling = vehicleFuelings.find(
      (f) => new Date(f.fuelingDate) < new Date(fueling.fuelingDate),
    );
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-12 gap-4 mb-3">
                <div className="col-span-4">
                  ...
                  <Input
                    ...
                    onBlur={(e) =>
                      e.target.value && searchVehicleByPlate(e.target.value)
                    }
                    placeholder="ABC1234"
                    maxLength={7}
            ...
            <CardContent className="pt-4">
              ...
              <div className="grid grid-cols-12 gap-4 mb-3">
                ...
                <div className="col-span-3">
                  ...
                  <Input
                    value={
                      formData.lastFuelingDate
                        ? format(formData.lastFuelingDate, "dd/MM/yyyy")
                        : "-"
                    }
                    readOnly
                    className="h-8 bg-slate-100 text-slate-600"
                ...
                <div className="col-span-4">
                  <Label className="text-xs font-medium">
                    Quilometragem (Último):
                  </Label>
                  <Input
                    value={
                      formData.lastKm
                        ? `${formData.lastKm.toLocaleString("pt-BR")} km`
                        : "-"
                    }
                    readOnly
                    className="h-8 bg-slate-100 text-slate-600"
              ...
              {formData.currentKm && formData.liters && formData.totalValue && (
                <div className="flex gap-4 mb-4">
                  <Badge variant="outline" className="text-xs">
                    Km Percorridos: {metrics.kmTraveled.toLocaleString("pt-BR")}{" "}
                    km
                  </Badge>
                  <Badge variant="outline" className="text-xs">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setFormData((prev) => ({ ...prev, createExpense: checked }))
                  }
                  disabled={isFormDisabled}
                />
        ...
        <DialogFooter className="bg-slate-100 -mx-6 -mb-6 px-6 py-3 mt-4">
          <div className="flex gap-2 w-full justify-end">
            ...
            <Button variant="outline" size="sm" disabled={editMode !== "none"}>
                  ...
                  {fuelings.map((f) => (
                    ...
                    >
                      <TableCell className="text-xs">
                        {format(f.fuelingDate, "dd/MM/yyyy")}
                      </TableCell>
                      ...
                      <TableCell className="text-xs">
                        {f.currentKm?.toLocaleString("pt-BR")}
                      </TableCell>
                      <TableCell className="text-xs">
                        {f.liters?.toFixed(2)}
                      </TableCell>
