...
export default function ReceivableReportPage() {
  ...
  const filteredReceivables = useMemo(() => {
    ...
    const createdStart = startOfDay(
      new Date(filters.createdStartDate + "T00:00:00"),
    );
    ...
    return allReceivables.filter((receivable) => {
      ...
      const createdAtMatch =
        createdAt >= createdStart && createdAt <= createdEnd;

      return dueDateMatch && createdAtMatch;
    });
  ...
  const loadReceivables = async () => {
    try {
      ...
      const data = await AccountsReceivable.filter(
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-slate-600">
            Análise detalhada das contas a receber por vencimento e lançamento
          </p>
        </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleFilterChange("dueStartDate", e.target.value)
                      }
                      className="bg-white"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleFilterChange("dueEndDate", e.target.value)
                      }
                      className="bg-white"
                    />
                  </div>
                    ...
                    {filteredReceivables.length > 0 ? (
                      filteredReceivables.map((receivable) => (
                        <TableRow key={receivable.id}>
                          ...
                          <TableCell>
                            {format(receivable.createdAt, "dd/MM/yyyy")}
                          </TableCell>
                          <TableCell>
                            {format(receivable.dueDate, "dd/MM/yyyy")}
                          </TableCell>
                          <TableCell>
                            {receivable.installmentNumber}/1
                          </TableCell>
                          <TableCell>
                            R$ {(receivable.amount || 0).toFixed(2)}
                          </TableCell>
                          <TableCell>
                            R$ {(receivable.paidAmount || 0).toFixed(2)}
                          </TableCell>
                      ...
                      <TableRow>
                        ...
                        >
                          Nenhuma conta a receber encontrada com os filtros
                          selecionados
                        </TableCell>
                      </TableRow>
                    )}
