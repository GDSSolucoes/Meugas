 ...
 * 
 * Generates TypeScript Entity classes for the frontend based on backend DTO files.
...
function parseDtoFile(entityName) {
  ...
  // Simple approach: find all property declarations
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      if (['class', 'export', 'import', 'const', 'let', 'var', 'function'].includes(propName)) {
        continue;
      }
      
      let type = typeStr.trim().split('|')[0].trim();
      
      console.log(`Found property: ${propName}${optional || ''}: ${type}`);
      properties.push({
        name: propName,
        type: type,
        required: optional !== '?',
  ...
  console.log(`Total properties found for ${entityName}: ${properties.length}`);
