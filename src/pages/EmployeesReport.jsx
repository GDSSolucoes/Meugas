...
export default function EmployeesReportPage() {
  ...
  const totalSalary = employees.filter(e => e.active).reduce((sum, e) => sum + (e.salary || 0), 0);
  const vendedores = employees.filter(e => e.position === 'vendedor' && e.active);
  const entregadores = employees.filter(e => e.position === 'entregador' && e.active);

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-800 mb-2">Relatório de Funcionários</h1>
          <p className="text-slate-600">Visão completa da equipe e custos com pessoal</p>
        </div>
                  ...
                  <TableBody>
                    {employees.map(employee => (
                      <TableRow key={employee.id}>
                        ...
                        <TableCell>
                          <Badge className={employee.active ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}>
