...
export default function AccountsReceivablePage({ onComplete }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [contasData, cashAccountsData, paymentTypesData, sectorsData, peopleData, employeesData] = await Promise.all([
        AccountsReceivable.filter({ company_id: user.company_id }, '-due_date'),
        CashAccount.filter({ company_id: user.company_id, active: true }),
        PaymentType.filter({ company_id: user.company_id, active: true }),
        Sector.filter({ company_id: user.company_id, active: true }),
        Person.filter({ company_id: user.company_id }),
  ...
  const handleQuickSacadoSearch = () => {
    if (tiposSacadoSelecionados === 0) {
      toast({ title: "Atenção", description: "Selecione pelo menos um tipo de sacado.", variant: "destructive" });
      return;
    }
  ...
  const handlePesquisarClick = () => {
    ...
    } else if (metodoPesquisa === 'codigo_venda') {
      setShowCodigoSearch(true);
            ...
            <CardContent className="p-4">
              <div className="grid grid-cols-12 gap-4">
                ...
                <div className="col-span-4 pr-4 border-r border-slate-200">
                  <h4 className="text-xs font-semibold text-slate-700 uppercase mb-3">Pesquisar</h4>
                  ...
                  <div className="mb-3">
                    ...
                    <div className="flex gap-3 mt-1">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        />
                        <label htmlFor="ponto_venda" className="text-xs">Pto. Venda</label>
                      </div>
                      <div className="flex items-center gap-1">
                        <Checkbox 
                          id="conveniada" 
                  ...
                  <div className="border-t border-slate-200 pt-3">
                    <Label className="text-xs font-medium mb-2 block">Pesquisar por:</Label>
                    <RadioGroup value={metodoPesquisa} onValueChange={setMetodoPesquisa} className="flex gap-3 mb-2">
                      <div className="flex items-center gap-1">
                        <RadioGroupItem value="codigo_venda" id="codigo_venda" />
                        <label htmlFor="codigo_venda" className="text-xs">Cód. Venda</label>
                      </div>
                      <div className="flex items-center gap-1">
                        <RadioGroupItem value="nota_fiscal" id="nota_fiscal" />
                        <label htmlFor="nota_fiscal" className="text-xs">Nota Fiscal</label>
                      </div>
                      <div className="flex items-center gap-1">
                ...
                <div className="col-span-5 px-4 border-r border-slate-200">
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <Select value={filtroRespCobranca} onValueChange={setFiltroRespCobranca}>
                        <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="todos">Todos</SelectItem>
                          {employees.map(e => (
                  ...
                  <div className="flex items-center gap-4 pt-2 border-t border-slate-200">
                    <div className="flex items-center gap-1">
                      ...
                      />
                      <label htmlFor="nao_paga" className="text-xs">Não Paga</label>
                    </div>
                    <div className="flex items-center gap-1">
                      <Checkbox 
                        id="paga" 
                      ...
                      />
                      <label htmlFor="paga" className="text-xs">Paga</label>
                    </div>
                    <div className="flex items-center gap-1">
                      <Checkbox 
                        id="em_cobranca" 
                  ...
                  <div className="border-t border-slate-200 pt-3 mt-3">
                    <div className="flex items-center gap-4">
                      ...
                      <div className={`flex flex-col gap-1 ${!usarPeriodo ? 'opacity-50 pointer-events-none' : ''}`}>
                        <div className="flex items-center gap-1">
                ...
                <div className="col-span-3 pl-4">
                  ...
                  <div className="border-t border-slate-200 pt-3">
                    ...
                    <RadioGroup value={ordenacao} onValueChange={setOrdenacao} className="flex flex-col gap-1">
                      <div className="flex items-center gap-1">
                      ...
                      <div className="flex items-center gap-1">
                        <RadioGroupItem value="valor" id="ord_valor" />
                        <label htmlFor="ord_valor" className="text-xs">Valor</label>
                      </div>
                    </RadioGroup>
                  </div>
              ...
              <Table>
                ...
                <TableBody>
                  ...
                  {filteredPeople.filter(p => {
                    if (!sacadoSearchTerm) return true;
                    const term = sacadoSearchTerm.toLowerCase();
                    return p.name?.toLowerCase().includes(term) || p.document?.toLowerCase().includes(term);
                  }).length === 0 && (
                    <TableRow>
              ...
              <Table>
                ...
                <TableBody>
                  ...
                  {getCodigoSearchData().filter(item => {
                    if (!codigoPesquisa) return true;
                    const term = codigoPesquisa.toLowerCase();
                    return item.label?.toLowerCase().includes(term) || item.extra?.toLowerCase().includes(term);
                  }).length === 0 && (
                    <TableRow>
                      <TableCell colSpan={4} className="text-center py-8 text-slate-500">
                        Nenhum registro encontrado
