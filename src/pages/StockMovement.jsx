...
export default function StockMovementPage() {
  ...
  useEffect(() => {
    const initializeAppData = async () => {
      ...
      try {
        ...
        if (!selectedSectorId) {
          // Check if a sector was already set by default
          setIsLoading(false);
        }
      }
    };
  ...
  const loadStockForSector = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const [allProducts, existingStocksForSector, allSectors] =
        await Promise.all([
      ...
      if (!currentSector) {
        console.warn(
          `Setor com ID ${selectedSectorId} não encontrado para a empresa ${companyId}.`,
        );
        setStocks([]);
      ...
      const existingProductIdsInSector = new Set(
        existingStocksForSector.map((s) => s.productId),
      );
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-slate-600">
            Gerencie a quantidade e data inicial do estoque por setor.
          </p>
        </div>
