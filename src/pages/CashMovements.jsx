...
import { 
  Search, X, LogOut, Printer, Plus, FolderOpen, Folder,
  TrendingUp, TrendingDown, Fuel, CreditCard, Factory, Store, User, Edit, Trash2,
  Check, Save, Calculator, AlertCircle, ArrowRight, Upload, Download
} from "lucide-react";
import {
  Dialog,
  DialogContent,
...
export default function CashMovements({ onComplete }) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const dinheiroPt = paymentTypesData.find(pt => pt.type === 'dinheiro' || pt.name?.toLowerCase().includes('dinheiro'));
      if (dinheiroPt) {
        setFormData(prev => ({
          ...prev,
  ...
  useEffect(() => {
    const updateFinancials = async () => {
      ...
      try {
        ...
        const receitas = periodMovements.filter(m => m.type === 'receita').reduce((sum, m) => sum + m.amount, 0);
        const despesas = periodMovements.filter(m => m.type === 'despesa').reduce((sum, m) => sum + m.amount, 0);
  ...
  const handleModificar = () => {
    if (!selectedMovement) {
      toast({ title: "Atenção", description: "Selecione um lançamento para modificar.", variant: "destructive" });
      return;
    }
  ...
  const handleExcluir = () => {
    if (!selectedMovement) {
      toast({ title: "Atenção", description: "Selecione um lançamento para excluir.", variant: "destructive" });
      return;
    }
  ...
  const confirmExcluir = async () => {
    ...
    try {
      const account = cashAccounts.find(a => a.id === selectedAccount);
      const balanceChange = selectedMovement.type === 'receita' 
  ...
  const handleSubmit = async () => {
    ...
    if (isInstallmentPayment && formData.installments > 1) {
      toast({ title: "Atenção", description: "Para pagamentos parcelados, use o botão 'Calcular' para gerar as parcelas.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      toast({ title: "Sucesso", description: selectedMovement ? "Lançamento atualizado!" : "Lançamento criado!" });
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col">
      ...
      <div className="bg-white border-b border-slate-300 p-4">
        <div className="max-w-full mx-auto">
          <h1 className="text-xl font-bold text-slate-800 mb-4">Lançamentos Financeiros</h1>
          ...
          <div className="flex flex-wrap items-end gap-4">
            ...
            <div className="ml-auto text-right">
              <Label className="text-xs font-medium text-slate-600">Saldo Anterior</Label>
                ...
                <Table>
                  <TableHeader className="bg-slate-100 sticky top-0">
                    <TableRow>
                      ...
                      <TableHead className="w-24 text-right text-xs">Gastar</TableHead>
                      <TableHead className="w-24 text-right text-xs">Receber</TableHead>
                      <TableHead className="w-28 text-right text-xs">Saldo</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {filteredMovements.length === 0 ? (
                      ...
                      filteredMovements.map((mov, idx) => {
                        const runningBalance = openingBalance + movements
                          .slice(0, movements.findIndex(m => m.id === mov.id) + 1)
                          .reduce((acc, m) => acc + (m.type === 'receita' ? m.amount : -m.amount), 0);
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell className="text-right text-xs font-mono text-green-600">
                              {mov.type === 'receita' ? formatCurrency(mov.amount) : '-'}
                            </TableCell>
                            <TableCell className={`text-right text-xs font-mono font-bold ${runningBalance >= 0 ? 'text-green-700' : 'text-red-700'}`}>
          ...
          <Card className="bg-white border-slate-300 overflow-hidden">
            ...
            <CardContent className="p-4 border-t border-slate-300">
              {activeTab === 'transferencia' ? (
                <div className={`space-y-3 ${isFormDisabled ? 'opacity-60' : ''}`}>
                  {/* Linha 1: De + Data */}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      >
                        ...
                        <SelectContent>
                          {sectorMasters.length > 0 && sectorMasters.map(sm => (
                            <SelectItem key={sm.id} value={sm.id}>{sm.name} (Master)</SelectItem>
                          ))}
                          {sectors.map(s => (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input 
                        type="text"
                        inputMode="decimal"
                        value={typeof transferData.amount === 'number' ? formatMoneyDisplay(transferData.amount) : transferData.amount}
                        onChange={(e) => handleMoneyChange(e, 'amount', setTransferData)}
                        onBlur={() => handleMoneyBlur('amount', setTransferData, transferData.amount)}
                        placeholder="0,00"
              ...
              <div>
                <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 ${isFormDisabled ? 'opacity-60' : ''}`}>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <Input 
                        type="text"
                        inputMode="decimal"
                        value={typeof formData.amount === 'number' ? formatMoneyDisplay(formData.amount) : formData.amount}
                        onChange={(e) => handleMoneyChange(e, 'amount', setFormData)}
                        onBlur={() => handleMoneyBlur('amount', setFormData, formData.amount)}
                        placeholder="0,00"
                        className={`h-8 border-slate-300 text-right ${disabledInputClass}`}
                        disabled={isFormDisabled}
                      />
                  ...
                  <div className="space-y-3">
                    <div className="text-right mb-4">
                      <Label className="text-xs font-medium text-slate-600">Saldo Atual...</Label>
                          ...
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                              ...
                              if (found) {
                                ...
                                setSacadoError('Código não encontrado. Use o botão Pesquisar.');
                              }
                            }
                          }}
                ...
                {isInstallmentPayment && (
                  <div className={`mt-4 pt-4 border-t border-slate-200 ${isFormDisabled ? 'opacity-50 pointer-events-none' : ''}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                      <div>
                        <Label className="text-xs font-medium">
                          Vcto. Entrada <span className="text-red-500">*</span>
                        </Label>
                        <Input 
                          type="date"
                      ...
                      <div>
                        <Label className="text-xs font-medium">Vl. Entrada</Label>
                        <div className="flex items-center gap-2">
                          <Input 
                            ...
                            disabled={isFormDisabled}
                          />
                          <span className="text-slate-700 font-bold text-base pb-0.5">+</span>
                        </div>
                      </div>
      ...
      <div className="bg-slate-200 border-t border-slate-300 p-2">
        <div className="flex flex-wrap gap-1 items-center">
          ...
          <Button 
            ...
            onClick={activeTab === 'transferencia' ? handleTransferSubmit : handleSubmit}
            type="button"
          >
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="py-4 text-center">
            <p className="text-lg mb-4">
              ...
              <span className="font-bold text-blue-600">{formatCurrency(transferData.amount)}</span>
            </p>
          ...
          <DialogFooter>
            ...
            <Button className="text-white hover:opacity-90" style={{ backgroundColor: '#223f61' }} onClick={confirmTransfer}>
              Confirmar
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={showSearchModal} onOpenChange={setShowSearchModal}>
        <DialogContent className="max-w-2xl max-h-[80vh]">
          <DialogHeader>
            <DialogTitle>
              Pesquisar {activeTab === 'gastar' ? 'Fornecedor' : (sacadoType === 'cliente' ? 'Cliente' : 'Ponto de Venda')}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="max-h-96 overflow-auto border rounded">
              <Table>
                ...
                <TableBody>
                  {people
                    .filter(p => activeTab === 'gastar' ? p.type === 'fornecedor' : p.type === sacadoType)
                    .filter(p => {
                  ...
                  {people.filter(p => activeTab === 'gastar' ? p.type === 'fornecedor' : p.type === sacadoType).length === 0 && (
                    <TableRow>
                      <TableCell colSpan={4} className="text-center text-slate-500 py-8">
                        Nenhum {activeTab === 'gastar' ? 'fornecedor' : (sacadoType === 'cliente' ? 'cliente' : 'ponto de venda')} cadastrado
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
            ...
            <div className="bg-slate-50 p-3 rounded-lg">
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <span className="font-bold ml-2">{formatCurrency(formData.amount)}</span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span className="font-bold ml-2">{formData.installments}x</span>
                </div>
              </div>
            </div>
              ...
              <Table>
                <TableHeader className="bg-slate-100 sticky top-0">
                  <TableRow>
                    ...
                    <TableHead className="w-36 text-xs text-right">Valor</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
            ...
            <div className="flex justify-between items-center pt-2 border-t">
              <div className="text-sm">
                ...
                <span className="font-bold ml-2 text-lg">
                  {formatCurrency(calculatedInstallments.reduce((sum, i) => sum + (Number(i.amount) || 0), 0))}
                </span>
              </div>
            </div>
          </div>
