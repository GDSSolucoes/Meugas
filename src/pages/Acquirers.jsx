...
export default function AcquirersPage() {
  ...
  const loadAcquirers = useCallback(async () => {
    try {
      ...
      toast({
        title: "Erro",
        description: "Falha ao carregar adquirentes.",
        variant: "destructive",
      });
    }
  }, [toast]);
  ...
  const handleDelete = async (acquirerId) => {
    if (window.confirm("Tem certeza que deseja excluir esta adquirente?")) {
      try {
        ...
        toast({
          title: "Sucesso",
          description: "Adquirente excluída com sucesso.",
        });
      } catch (error) {
        console.error("Erro ao excluir adquirente:", error);
        toast({
          title: "Erro",
          description: "Não foi possível excluir a adquirente.",
          variant: "destructive",
        });
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        await Acquirer.create({ ...payload, createdByName: user.name });
      }
      setShowForm(false);
      resetForm();
      loadAcquirers();
      toast({
        title: "Sucesso",
        description: `Adquirente ${isEditing ? "atualizada" : "salva"} com sucesso.`,
      });
    } catch (error) {
      console.error("Erro ao salvar adquirente:", error);
      toast({
        title: "Erro",
        description: "Não foi possível salvar a adquirente.",
        variant: "destructive",
      });
    }
  };
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setCurrentAcquirer((prev) => ({
                          ...prev,
                          settlementDays: parseInt(e.target.value) || 1,
                        }))
                      }
                    />
                  </div>
