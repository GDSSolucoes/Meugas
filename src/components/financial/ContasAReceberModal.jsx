...
export default function ContasAReceberModal({
  ...
  const applyFiltersAndShow = () => {
    const filtered = contas
      .filter((c) => {
        ...
        const isVencida =
          c.status === "pendente" &&
          isBefore(c.dueDate, startOfDay(new Date()));
  ...
  const allFilteredContas = contas
    .filter((c) => {
      // Filtro de status
      const isVencida =
        c.status === "pendente" && isBefore(c.dueDate, startOfDay(new Date()));
      const isPaga = c.status === "pago";
      const isEmCobranca = c.status === "emCobranca";
      ...
      if (!statusContas.pagas && isPaga) return false;
      if (!statusContas.emCobranca && isEmCobranca) return false;
      if (statusContas.naoPagas && !isPaga && !isEmCobranca) {
        /* ok */
      }
      ...
      if (codigoPesquisa) {
        if (
          ...
          !c.saleId?.includes(codigoPesquisa)
        )
          return false;
        if (
          metodoPesquisa === "notaFiscal" &&
          !c.nfeNumber?.includes(codigoPesquisa)
        )
          return false;
        if (
          metodoPesquisa === "documento" &&
          !c.personDocument?.includes(codigoPesquisa)
        )
          return false;
      }
  ...
  const handleQuickSearch = () => {
    ...
    const results = contas.filter((c) => {
      ...
      if (metodoPesquisa === "codigoVenda") {
        return (
          c.saleId?.toLowerCase().includes(searchTerm) ||
      ...
      if (metodoPesquisa === "documento") {
        return (
          c.personDocument?.toLowerCase().includes(searchTerm) ||
    ...
    if (results.length === 0) {
      toast({
        ...
        description: "Nenhuma conta encontrada com esse código.",
        variant: "destructive",
      });
      setSearchResults([]);
      setDisplayedContas([]);
          ...
          <div className="grid grid-cols-12 gap-4 p-4 bg-slate-50 rounded-lg border">
            ...
            <div className="col-span-3 space-y-3">
                  ...
                  {searchResults.map((conta) => (
                    ...
                    >
                      ...
                      <div className="text-slate-500">
                        {formatCurrency(conta.amount)} - Venc:{" "}
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <label htmlFor="usarPeriodo" className="text-xs font-medium">
                    Período
                  </label>
                </div>
                <div
              ...
              <div className="space-y-1">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <label htmlFor="naoPagas" className="text-xs">
                      Não Pagas
                    </label>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <label htmlFor="pagas" className="text-xs">
                      Pagas
                    </label>
                  </div>
                  <div className="flex items-center gap-2">
            ...
            <div className="col-span-2 space-y-3">
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <label htmlFor="ordData" className="text-xs">
                    Data
                  </label>
                </div>
                <div className="flex items-center gap-2">
            ...
            <div className="col-span-2 flex items-end">
              <Button
                ...
                className="w-full h-8 text-xs gap-1 bg-slate-800 hover:bg-slate-900"
              >
                ...
                <div className="flex items-center justify-center h-48 text-slate-500 text-sm p-8">
                  <div className="text-center">
                    ...
                    <p className="text-xs mt-1">
                      ou digite um código e clique no ⚡
                    </p>
                  </div>
                </div>
              ) : (
            ...
            <div className="space-y-4">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      const results = contas.filter((c) => {
                        ...
                        return (
                          c.personDocument?.toLowerCase().includes(term) ||
              ...
              <div className="max-h-80 overflow-auto border rounded">
                <Table>
                  ...
                  <TableBody>
                    {(searchResults.length > 0 ? searchResults : contas).map(
                      (conta) => (
                        <TableRow
                          key={conta.id}
              ...
              <p className="text-xs text-slate-500">
                Dê duplo clique para selecionar uma conta
              </p>
            </div>
            <DialogFooter>
              <Button
                ...
                <div className="mt-4 p-3 bg-slate-50 rounded-lg text-sm space-y-2 max-h-40 overflow-auto">
                  {getSelectedContasForAction().map((conta) => (
                    <div key={conta.id} className="border-b pb-2 last:border-0">
                      <p>
                        <strong>Sacado:</strong> {conta.personName}
                      </p>
                      <p>
