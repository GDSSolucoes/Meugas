...
import { companies, facilitadores, persons, products, sales } from '../../database/schemas'
...
export class FiscalService {
  ...
  async cancelFiscalNote(dto: CancelFiscalNoteDto, user: UserContext) {
    ...
    if (sale.companyId !== user.companyId) {
      throw new NotFoundException('Venda não encontrada para a empresa do usuário')
    }

    const notaType = dto.tipoNota === 'nfe' ? 'nfe' : 'nfce'
    const noteKey = dto.tipoNota === 'nfe' ? sale.nfeKey : sale.nfceKey
    if (!noteKey) {
      throw new BadRequestException(`Chave da ${notaType.toUpperCase()} não encontrada para esta venda`)
    }
    ...
    if (!ambiente) {
      throw new BadRequestException(`Ambiente fiscal para ${notaType.toUpperCase()} não configurado`) 
    }

    const token = await this.getNuvemFiscalToken()
    const baseUrl = ambiente === 'producao' ? 'https://api.nuvemfiscal.com.br' : 'https://api.sandbox.nuvemfiscal.com.br'
    ...
    try {
      ...
      throw new InternalServerErrorException('Erro ao interpretar resposta da Nuvem Fiscal: ' + responseText)
    }

    if (!response.ok) {
      throw new InternalServerErrorException(responseData?.error || responseData || 'Erro ao cancelar nota fiscal na Nuvem Fiscal')
    }
  ...
  async downloadDanfe(dto: DownloadFiscalDto, user: UserContext) {
    ...
    if (!key) {
      throw new BadRequestException(`Chave da ${dto.type.toUpperCase()} não encontrada`) 
    }
    ...
    if (!ambiente) {
      throw new BadRequestException(`Ambiente fiscal para ${dto.type.toUpperCase()} não configurado`)
    }
    ...
    if (!response.ok) {
      throw new InternalServerErrorException(result?.error || 'Erro ao baixar DANFE da Nuvem Fiscal')
    }
  ...
  async downloadXml(dto: DownloadFiscalDto, user: UserContext) {
    ...
    const result = await response.json()
    if (!response.ok) {
      throw new InternalServerErrorException(result?.error || 'Erro ao baixar XML da Nuvem Fiscal')
    }
  ...
  async searchAddressByStreet(dto: SearchAddressDto) {
    ...
    if (!response.ok) {
      ...
      throw new InternalServerErrorException(`Erro ao buscar endereço: ${text}`)
    }

    return response.json()
  }

  private async emitFiscal(dto: EmitFiscalDto, user: UserContext, noteType: 'nfe' | 'nfce') {
    const sale = await this.getSale(dto.saleId)
    if (!sale) {
      throw new NotFoundException('Venda não encontrada')
    }
    ...
    if (noteType === 'nfe' && facilitador.modeloFiscal !== '55') {
      throw new BadRequestException('Facilitador selecionado não é para NF-e (modelo 55)')
    }

    if (noteType === 'nfce' && facilitador.modeloFiscal !== '65') {
      throw new BadRequestException('Facilitador selecionado não é para NFC-e (modelo 65)')
    }
    ...
    if (!company.parametrosFiscais) {
      throw new BadRequestException('Parâmetros fiscais não configurados. Acesse Cadastros → Empresas e configure os parâmetros fiscais.')
    }

    const params = company.parametrosFiscais as any
    if (noteType === 'nfe' && !params.emitirNfe) {
      throw new BadRequestException('Emissão de NF-e não está habilitada nos parâmetros fiscais.')
    }

    if (noteType === 'nfce' && !params.emitirNfce) {
      throw new BadRequestException('Emissão de NFC-e não está habilitada nos parâmetros fiscais.')
    }

    if (!params.cnpj || !params.razaoSocial) {
      throw new BadRequestException('CNPJ e Razão Social são obrigatórios nos parâmetros fiscais.')
    }
    ...
    const numeroSequencial = noteType === 'nfe' ? params.numeroInicialNfe || 1 : params.numeroInicialNfce || 1
    ...
    try {
      ...
      throw new InternalServerErrorException('Erro ao processar resposta da Nuvem Fiscal: ' + responseText)
    }

    if (!response.ok) {
      throw new InternalServerErrorException(responseData?.error || responseData || 'Erro ao emitir nota fiscal na Nuvem Fiscal')
    }
    ...
    await this.db.update(sales).set(updatePayload).where(eq(sales.id, dto.saleId))

    if (responseData.status === 'autorizada' || responseData.status === 'autorizado') {
      const nextNumberField = noteType === 'nfe' ? 'numeroInicialNfe' : 'numeroInicialNfce'
    ...
    return {
      success: true,
      message: responseData.status === 'autorizada' || responseData.status === 'autorizado'
  ...
  private async getCompany(companyId: string) {
    const rows = await this.db.select().from(companies).where(eq(companies.id, companyId))
    return rows[0] || null
  }

  private async getCustomer(personId: string) {
    const rows = await this.db.select().from(persons).where(eq(persons.id, personId))
    return rows[0] || null
  }

  private async getFacilitador(facilitadorId: string) {
    const rows = await this.db.select().from(facilitadores).where(eq(facilitadores.id, facilitadorId))
    return rows[0] || null
  }
  ...
  private getAmbiente(parametros: any, noteType: 'nfe' | 'nfce') {
    ...
    return noteType === 'nfe' ? parametros.ambienteNfe || 'homologacao' : parametros.ambienteNfce || parametros.ambienteNfe || 'homologacao'
  }
  ...
  private async getNuvemFiscalToken() {
    ...
    if (!clientId || !clientSecret) {
      throw new InternalServerErrorException('Credenciais da Nuvem Fiscal não configuradas')
    }
    ...
    try {
      ...
      throw new InternalServerErrorException('Erro ao processar autenticação da Nuvem Fiscal: ' + body)
    }

    if (!response.ok) {
      throw new InternalServerErrorException(tokenData?.error || 'Erro ao autenticar na Nuvem Fiscal')
    }

    if (!tokenData.access_token) {
      throw new InternalServerErrorException('Token de autenticação da Nuvem Fiscal não retornado')
    }
  ...
  private buildPayload(options: {
    ...
    const { sale, customer, company, facilitador, params, noteType, numeroSequencial } = options

    const telefoneEmitente = this.cleanPhone(company.phone)
    const telefoneDestinatario = this.cleanPhone(Array.isArray(customer.phone) ? customer.phone[0] : customer.phone)
    ...
    const ide: any = {
      ...
      serie: parseInt(noteType === 'nfe' ? params.serieNfe || '1' : params.serieNfce || '1', 10) || 1,
    ...
    return {
      ambiente: this.getAmbiente(params, noteType) === 'producao' ? 'producao' : 'homologacao',
      ...
      infNFe: {
        ...
        transp: { modFrete: 9 },
        pag: this.mapearPagamentos(sale.paymentMethods || [], sale.totalAmount || 0),
        infAdic: {
          infCpl: [params.observacoesNfe || '', sale.notes || ''].filter(Boolean).join('\n') || 'Documento emitido por ME/EPP optante pelo Simples Nacional.',
        },
        infRespTec: {
          CNPJ: '04954377000130',
